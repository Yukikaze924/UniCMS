...
export default function MysqlTable({ table, exportBtn }: { table: string; exportBtn: any }) {
    return (
        <div>
            {table === 'users' ? (
                <UsersTable />
...
function Table({ table, exportBtn }) {
    ...
    function copyToClipboard(_e: MouseEvent) {
        if (rows) {
            ...
            copy(JSON.stringify(selectedRowsArray, null, 4));
                                    ...
                                    <td className="w-4 p-4">
                                        <div className="flex items-center">
                                            <input
                                                ...
                                                onChange={(e) =>
                                                    setSelectedRows((prev) => ({ ...prev, [index]: e.target.checked }))
                                                }
                                            />
...
function UsersTable() {
    ...
    function submitUserUpdate() {
        ...
        fetch(`/api/util/db/updateUser?args_0=${userEditing.uid}&args_1=${JSON.stringify(formatObj)}`).finally(() => {
            setEditModal(false);
            setUserEditing(null);
            fetchUsers();
        });
                        ...
                        {users.map((user: any, index: number) => {
                            return (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4">{truncateWithEllipsis(user.name, 20)}</td>
                                    <td className="px-6 py-4">{maskPassword(user.password, 15)}</td>
                                    <td className="px-6 py-4">{truncateWithEllipsis(user.avatar, 30)}</td>
                                    ...
                                    <div className="grid gap-4 mb-4 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                                                    setUserEditing((prev: any) => ({ ...prev, name: e.target.value }))
                                                }
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                                                    setUserEditing((prev: any) => ({ ...prev, email: e.target.value }))
                                                }
                                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-yellow-600 focus:border-yellow-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-yellow-500 dark:focus:border-yellow-500"
                            ...
                            {movies.map((movie: any, index: number) => {
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-3 text-gray-900 dark:text-white">{movie.id}</td>
                                        <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                                            {truncateWithEllipsis(movie.title, 32)}
                                        </td>
                                        <td className="px-6 py-4 text-gray-900 dark:text-white">{movie.type}</td>
                                        <td className="px-6 py-3 text-gray-900 dark:text-white">{movie.rating}</td>
                                        ...
                                        <td className="px-6 py-3 text-gray-900 dark:text-white">{movie.casts}</td>
                                        <td className="px-6 py-3 text-gray-900 dark:text-white">{movie.director}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
