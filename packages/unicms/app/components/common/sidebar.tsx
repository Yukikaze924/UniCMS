...
const unselectedStyleExpanded =
    'flex gap-4 bg-white hover:bg-neutral-50 rounded-xl font-semibold text-sm text-gray-900 py-3 px-4';
const selectedStyleExpanded = 'flex gap-4 bg-indigo-50 rounded-xl font-semibold text-sm text-indigo-600 py-3 px-4';
const unselectedStyle =
    'flex justify-center items-center bg-white hover:bg-neutral-50 rounded-xl font-semibold text-sm text-neutral-800 w-full aspect-square p-3';
const selectedStyle =
    'flex justify-center items-center bg-indigo-50 rounded-xl font-semibold text-sm text-indigo-600 w-full aspect-square p-3';
...
export default function Sidebar() {
    ...
    useEffect(() => {
        if (sidebarExpanded === true) setValue({ ...value, sidebarExpanded: true });
        else if (sidebarExpanded === false) setValue({ ...value, sidebarExpanded: false });
    }, [sidebarExpanded]);

    return (
        <>
            <aside
                className={`z-10 max-md:hidden shadow-md bg-white h-screen transition-all duration-200 ease-in-out ${sidebarExpanded ? 'w-64' : 'w-20'}`}
            >
                <div className={`flex flex-col justify-between h-full ${sidebarExpanded ? 'w-64' : 'w-20'}`}>
                    <div className="flex-grow">
                        {sidebarExpanded ? (
                            <div className="flex flex-row justify-center items-center space-x-1.5 px-4 py-6 border-b">
                                ...
                                >
                                    ...
                                    <g id="SVGRepo_iconCarrier">
                                        <g>
                                            <path d="M128,0 C57.3281853,0 0,57.3281853 0,128 C0,198.671815 57.3281853,256 128,256 C198.671815,256 256,198.671815 256,128 C256,57.3281853 198.671815,0 128,0"></path>
                                            <path
                            ...
                            <div
                                className="flex justify-center items-center w-full border-b"
                                style={{ aspectRatio: 1 }}
                            >
                                <svg
                        ...
                        <div className="p-2 mt-2">
                            <ul className={sidebarExpanded ? 'space-y-2' : '-space-y-2'}>
                                <SidebarNavItem expanded={sidebarExpanded} current={current} value="dashboard">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="currentColor"
                                ...
                                <SidebarNavItem expanded={sidebarExpanded} current={current} value="content">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="currentColor"
                                        className="w-5 h-5 text-lg"
                                ...
                                <SidebarNavItem expanded={sidebarExpanded} current={current} value="explorer">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="currentColor"
                                        className="w-5 h-5 text-lg"
                                ...
                                <SidebarNavItem expanded={sidebarExpanded} current={current} value="database">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="currentColor"
                                        className="w-5 h-5 text-lg"
                                ...
                                <SidebarNavItem expanded={sidebarExpanded} current={current} value="market">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="currentColor"
                                        className="w-5 h-5 text-lg"
                    ...
                    >
                        <button
                            type="button"
                            className="inline-flex items-center justify-center h-9 px-4 rounded-xl bg-gray-900 text-gray-300 hover:text-white text-sm font-semibold transition cursor-pointer"
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="1em"
        ...
        <>
            <li className={`${expanded ? '' : 'w-full p-2'}`}>
                {expanded ? (
                    ...
                    <Link to={`/${value}`} className={current === value ? selectedStyle : unselectedStyle}>
                        {children}
                    </Link>
                )}
            </li>
