...
export class PluginManager {
    ...
    public async load(key: string) {
        ...
        if (plugin && plugin.module) return plugin.module;
    }
    ...
    public async buildAsync() {
        ...
        try {
            const dirStat = await fs.stat(this.pluginsDirPath);
            if (!dirStat.isDirectory()) {
                return this; // If it's not a directory, return early
            }
        } catch (err) {
            return this; // Directory does not exist, return early
        }

        await fs.readdir(this.pluginsDirPath).then(async (dirnames) => {
